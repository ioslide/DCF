...
class randomSeverityImageNetcDataset(Dataset):
...
class randomSeverityImageNetcbarDataset(Dataset):
    def __init__(self, n_examples, severities, corruptions, data_dir):
        self.n_examples = n_examples
        self.severities = severities
        self.corruptions = corruptions
...
class randomSeverityImageNet3dccDataset(Dataset):
    def __init__(self, n_examples, severities, corruptions, data_dir):
        self.n_examples = n_examples
        self.severities = severities
        self.corruptions = corruptions
...
class randomSeverityImageNetcbarDataset(Dataset):
    def __init__(self, n_examples, severities, corruptions, data_dir):
        ...
        data_folder_path = {}
        for severity in severities:
            data_folder_path[severity] = Path(data_dir) / CORRUPTIONS_DIR_NAMES[BenchmarkDataset.imagenet][ThreatModel.corruptions_c_bar] / self.corruptions[0] / str(severity)
...
def load_imagenetc_w_random_severity(
    ...
    imagenet_c_dataset = randomSeverityImageNetcDataset(
...
def load_imagenetcbar_w_random_severity(
    n_examples,
    severities,
    data_dir,
    shuffle,
    ...
    imagenet_c_bar_dataset = randomSeverityImageNetcbarDataset(
        n_examples=n_examples,
        severities=severities,
        corruptions=corruptions,
        data_dir=data_dir
...
def load_imagenet3dcc_w_random_severity(
    ...
    imagenet_c_dataset = randomSeverityImageNet3dccDataset(
        n_examples=n_examples,
        severities=severities,
        corruptions=corruptions,
        data_dir=data_dir
...
def load_imagenetc(
    ...
    data_folder_path = Path(data_dir) / CORRUPTIONS_DIR_NAMES[BenchmarkDataset.imagenet][ThreatModel.corruptions] / corruptions[0] / str(severity)

    if os.path.exists(f"{data_folder_path}/data.pt"):
