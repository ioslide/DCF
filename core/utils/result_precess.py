...
class AvgResultProcessor:
    ...
    def info(self,frame_results=None):
        ...
        for c in range(self.num_class):
            ...
            # t.add_row([
            #     self.label2name[c], 
            #     f"{self.result_per_class[c] * 100:.2f}", 
            #     f"{(1 - self.result_per_class[c]) * 100:.2f}"
            # ])
            all_results[self.label2name[c]] = self.result_per_class[c] * 100

            # frame_results[self.label2name[c]] = self.result_per_class[c] * 100

        # avg is the total avg
        avg = sum(self.all_accurate) / len(self.all_accurate)
        # t.add_row(["Total Avg", f"{avg * 100:.2f}", f"{(1 - avg) * 100:.2f}"])
        all_results["Avg"] = avg * 100
